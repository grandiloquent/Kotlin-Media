...
class CircleImageView : ImageView {
    ...
    constructor(context: Context, color: Int) : super(context) {
        val shadowYOffset = Values.density * Y_OFFSET
        val shadowXOffset = Values.density * X_OFFSET
        mShadowRadius = (Values.density * SHADOW_RADIUS).toInt()
        val circle: ShapeDrawable
        if (Build.VERSION.SDK_INT >= 21) {
            circle = ShapeDrawable(OvalShape())
            this.elevation = SHADOW_ELEVATION * Values.density
        } else {
        ...
        Values.setBackground(this, circle)
    }
    companion object {
    ...
    fun setBackgroundColorRes(colorRes: Int) {
        setBackgroundColor(Values.getColor(colorRes))
    }
