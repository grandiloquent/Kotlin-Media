...
import com.euphoria.psycho.elf.common.calculateScreenOrientation
import com.euphoria.psycho.elf.common.hideSystemUI
import com.euphoria.psycho.elf.common.showSystemUI
...
import euphoria.psycho.common.Values.navigationBarHeight
import euphoria.psycho.common.Values.navigationBarWidth
import euphoria.psycho.media.R
import euphoria.psycho.media.getParentFilePath
import euphoria.psycho.media.inRange
import euphoria.psycho.media.toUri
import java.io.File
import java.util.*
...
class PlayerActivity : AppCompatActivity(), TimeBar.OnScrubListener,
        Player.EventListener, VideoListener, PlaybackPreparer, View.OnLayoutChangeListener,
        View.OnTouchListener,
        TextOutput {
    private var mIsHasBar = false
...
//            }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        //mTracker.e("[onCreate]")
        Values.context = this.applicationContext
    ...
    override fun onTouch(view: View?, event: MotionEvent): Boolean {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (mIsChangeVolume) {
                    dy = -dy
                    val max = Values.maxMusicVolume
                    val dv = max * dy * 3 / mScreenHeight
                    Values.musicVolume = mVolume + dv
                }
    ...
    private fun renameVideo(path: String?) {
        mPlayer?.let {
            ...
            dialog(this, path.getFilenameFromPath(), getString(R.string.menu_rename_file), true) {
                renameFile(this, File(path), File(path.getParentPath(), it.toString()))
            }
        }
    }
